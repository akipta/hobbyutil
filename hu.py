...
This script will copy source files from various locations in my data
directories to the hobbyutil Mercurial repository.  This repository is
stored at https://code.google.com/p/hobbyutil/ and is now the
preferred method for users who want to get the various files.

The script lets me modify the primary source files in their normal
locations; when the script is run, changed files are copied into the
repository.  When it's deemed appropriate, a push to Google's
repository is used to update the online repository.

Note this script contains both the code and the data to update the
...
Commands:
    dup         Show files that are duplicates (candidates for softlinks)
    dump        Dump the program's data to stdout
    ignored     Show ignored packages
    list        Show the project names and descriptions
    make        Copy any changed files to the repository
    outlaw      Show files present which aren't in package data
    show        Show which files have changed
    web         Create the project's main web page

Options:
    -F      Remove directories forcibly
    -i      In listing, include ignored projects
...
def ParseCommandLine(d):
    d["-F"]     = False     # If true, remove directories forcibly
    d["-i"]     = False     # Show ignored stuff in dump
    d["-s"]     = False     # If true, no descriptions for 'list'
    d["-q"]     = False
    d["show"]   = False     # If true, only show what will be copied
    if len(sys.argv) < 2:
        Usage(d)
    try:
        optlist, args = getopt.getopt(sys.argv[1:], "Fisq")
